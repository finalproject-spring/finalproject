...
.cyan {background: rgba(0, 255, 255, 1); }
.lime {background: rgba(205, 220, 57, 100);}
.amber {background: rgba(255, 152, 0, 100);}
.grey {background: rgba(158, 158, 158, 100);}
...
.entry.white:after {background: rgba(255, 255, 255, 1);}
.entry.cyan:after {background: rgba(0, 255, 255, 1); }
.entry.lime:after {background: rgba(205, 220, 57, 100);}
.entry.amber:after {background: rgba(255, 152, 0, 100);}
.entry.grey:after {background: rgba(158, 158, 158, 100);}
	...
	!function() {
		  ...
		  Calendar.prototype.drawMonth = function() {
		     ...
		     this.events.forEach(function(ev) {
			 	 ...
			 	 var inputEvent = new Date(ev.event)
			     ev.date = self.current.clone().month(inputEvent.getMonth()).date(inputEvent.getDate());
		  ...
		  Calendar.prototype.drawEvents = function(day, element) {
		    if(day.month() === this.current.month()) {
		      ...
		      todaysEvents.forEach(function(ev) {
		    	...
		    	if((day.month() + 1) == (start.getMonth() + 1)){
		    		if(day.date() == start.getDate()){
		    			var evSpan = createElement('span', ev.color, '', 'first');
		    	...
		    	} else if((day.month() + 1) == (end.getMonth() + 1)){
		    		if(day.date() == end.getDate()){
		    			var evSpan = createElement('span', ev.color, '', 'end');
		 		        element.appendChild(evSpan);
		    		}else{
		    			var evSpan = createElement('span', ev.color, '', 'middle');
		...
		!function() {
		  ...
		  <c:forEach items="${fund }" var="fund">
		      var start = new Date("${fund.funding_start}")
		      var end = new Date("${fund.funding_end}")	
		      var dateDiff = Math.ceil((end.getTime()-start.getTime())/(1000*3600*24));
		      var color = ['orange', 'yellow', 'green', 'blue', 'white', 'cyan', 'lime', 'amber', 'grey'];
	      ...
	      <c:forEach items="${event }" var="event">
		      var start = new Date("${event.event_start}")
		      var end = new Date("${event.event_end}")	
		      var dateDiff = Math.ceil((end.getTime()-start.getTime())/(1000*3600*24));
		      var color = ['orange', 'yellow', 'green', 'blue', 'white', 'cyan', 'lime', 'amber', 'grey'];
		      var random = parseInt((Math.random() * 9));
