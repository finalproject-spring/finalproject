-- 댓글개수
SELECT q.*, (SELECT COUNT(*) FROM REPLY r WHERE r.board_no = q.board_no) AS rp
FROM BOARD q
	...
	FROM (SELECT BOARD_NO, BOARD_ID, BOARD_TITLE, MEMBER_NO, BOARD_DATE, BOARD_SECRET, ROW_NUMBER() OVER(ORDER BY BOARD_NO DESC) AS rn FROM BOARD WHERE BOARD_CATEGORY='문의') bd
